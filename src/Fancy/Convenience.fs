...
module Convenience
    ...
    let addHeaders (headers: (string * string) list) (response:Response) = 
        headers
        |> Seq.iter (fun h -> match h with | (key, value) -> response.Headers.Add(key, value))
        response
    ...
    let addCookieWithExpr name value (expires:DateTime) (response:Response) = 
